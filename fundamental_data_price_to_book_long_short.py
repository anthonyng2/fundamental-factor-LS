...
3. Go short in the bottom 100 stocks by dividend yield.
...
def make_pipeline():
    ...
    top_pbr_stocks = pbr.top(100, mask=universe)
    bottom_pbr_stocks = pbr.bottom(100, mask=universe)
    
    # Screen to include only securities tradable for the day
    securities_to_trade = (top_pbr_stocks | bottom_pbr_stocks)
    
    pipe = Pipeline(
              columns={
                'div_yield': pbr,
                'longs': top_pbr_stocks,
                'shorts': bottom_pbr_stocks,
              },
...
def before_trading_start(context, data): 
    ...
    # The stocks that we want to short.
...
def rebalance(context, data):
    ...
    if (len(context.longs) > 0) and (len(context.shorts) > 0):
        ...
        # Open short positions in our stocks.
