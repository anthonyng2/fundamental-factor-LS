...
def make_pipeline():
    ...
    bottom_div_yield_stocks = div_yield.bottom(100, mask=universe)
    
    # Screen to include only securities tradable for the day
    securities_to_trade = (top_div_yield_stocks | bottom_div_yield_stocks)
    
    pipe = Pipeline(
              columns={
                'div_yield': div_yield,
                'longs': top_div_yield_stocks,
                'shorts': bottom_div_yield_stocks,
              },
